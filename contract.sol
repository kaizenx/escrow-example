...
contract EscrowNFT is ERC721Burnable, ERC721Enumerable, Ownable {
    ...
    function mint(address _recipient, uint256 _amount, uint256 _matureTime) public onlyOwner returns (uint256) {
      _mint_(_recipient, tokenCounter);
      // set values
    ...
    // boilerplate
    // return contract's address on the blockchain
    function contractAddress() public view returns (address) {
        return address(this);
    }
    // specify that we specify that the versions of _beforeTokenTransfer() and supportsInterface() provided by ERC721Enumerable should be preferred. because ambiguities introduced by Solidity's multiple inheritance. Because we're using two different ERC-721 extensions, the base ERC721 contract is inherited twice with different functions added and overridden. 
